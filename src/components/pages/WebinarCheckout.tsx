...
const WebinarCheckout = () => {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      router.push(`/webinar-registration/${id}`);
    }
  }, [id, router]);
  ...
  const onSubmit = (values: z.infer<typeof paymentFormSchema>) => {
    ...
    setTimeout(() => {
      ...
      // Store payment completion status in localStorage
      localStorage.setItem('webinarPaymentCompleted', 'true');
      router.push(`/webinar-confirmation?id=${webinar.id}`);
    }, 2000);
  };
  ...
  return (
    <div className="container mx-auto px-4 py-16 max-w-4xl">
      <button
        onClick={() => router.push(`/webinar-registration/${id}`)}
