...
export const getCourse = async (slug: string) => {
  ...
  const raw = response.data?.data?.courses?.[0];
  if (!raw) return [];
  ...
  return [{
    ...
    level: raw.level,
    on_sale: raw.on_sale,
    has_certificate: raw.has_certificate,
    ...
    image: raw.image_url?.url ? backendUrl + raw.image_url.url : null,
    enrolled_avatars: raw.enrolled_students?.enrolled_avatars?.map((avatar: any) => ({
      url: backendUrl + avatar.url,
      name: avatar.name
    })),
    total_enrolled: raw.enrolled_students?.total_enrolled,
    rating: raw.rating?.overall_rating,
    total_rated: raw.rating?.total_rated,
